...
export const GET: APIRoute = ({ site }) => {
  ...
  const lines = [
    ...
    "",
    "## Core Tools",
    ...Object.values(LANGUAGES).map(language => `- ${language.name} Formatter: ${origin}/${language.id}`),
    "",
    "## Utility Tools",
    ...UTILITY_TOOLS.map(tool => `- ${tool.name}: ${origin}/tools/${tool.slug}`),
    "",
    "## SEO Guides",
    ...SEO_CONTENT_PAGES.map(page => `- ${page.h1}: ${origin}${page.canonicalPath}`),
  ];
